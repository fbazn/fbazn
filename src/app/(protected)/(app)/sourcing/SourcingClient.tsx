            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <div key={label} className="rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-2.5 text-center">
                  ...
                  <p className="mt-0.5 text-xs font-semibold font-mono text-[rgb(var(--text))] truncate">{value}</p>
                </div>
              ))}
            </div>
...
export default function SourcingClient({ initialItems }: Props) {
  ...
  const [items, setOptimistic] = useOptimistic(
    ...
    (
      ...
      action:
        ...
        | { type: "update"; id: string; fields: Partial<SourcingItemRow> },
    ) => {
