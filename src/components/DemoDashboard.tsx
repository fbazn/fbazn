...
export default function DemoDashboard() {
  ...
  const addSource = (e:React.FormEvent)=>{ e.preventDefault(); const s={id:`s${Date.now()}`,...newSource} as SourcingProduct; setSourcing(prev=>[...prev,s]); setShowModal(false); setNewSource({wholesaler_id:'all',name:'',box_price:0,items_in_box:0,wholesale_price:0,asin:'',sell_price:0,amazon_fees:0}) }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              {hubCards.map(c=>(
                <div key={c.title} className="bg-white shadow rounded overflow-hidden">
                  ...
                  <div className="p-4 space-y-1 text-sm bg-gray-50">
                    {Object.entries(c.metrics).map(([k,v])=>(<div key={k}><strong>{k.replace(/([A-Z])/g,' $1')}:</strong> {v}</div>))}
                  </div>
                </div>
              ))}
            </div>
        ...
        {view==='wholesalers'&&(
          <section>
            ...
            <form onSubmit={addWholesaler} className="flex gap-2 mb-4">
              <input className="flex-1 border px-2 py-1 rounded" placeholder="Name" value={newWhName} onChange={e=>setNewWhName(e.target.value)}/>
              <input className="flex-1 border px-2 py-1 rounded" placeholder="Link" value={newWhLink} onChange={e=>setNewWhLink(e.target.value)}/>
        ...
        {view==='products'&&(
          <section>
            ...
            <form onSubmit={addProduct} className="space-y-2 mb-4">
              <input className="w-full border px-2 py-1 rounded" placeholder="Name" value={newProd.name} onChange={e=>setNewProd({...newProd,name:e.target.value})}/>
              <div className="flex gap-2">
                <input className="flex-1 border px-2 py-1 rounded" placeholder="Link" value={newProd.product_link} onChange={e=>setNewProd({...newProd,product_link:e.target.value})}/>
                <input type="number" className="w-24 border px-2 py-1 rounded" placeholder="Qty" value={newProd.qty_in_box} onChange={e=>setNewProd({...newProd,qty_in_box:+e.target.value})}/>
                <input type="number" className="w-24 border px-2 py-1 rounded" placeholder="Price" value={newProd.purchase_price} onChange={e=>setNewProd({...newProd,purchase_price:+e.target.value})}/>
              </div>
              <div className="flex gap-2 items-center">
                <input className="border px-2 py-1 rounded" placeholder="ASIN" value={newProd.asin} onChange={e=>setNewProd({...newProd,asin:e.target.value})}/>
                <select className="border px-2 py-1 rounded" value={newProd.wholesaler_id} onChange={e=>setNewProd({...newProd,wholesaler_id:e.target.value})}>
                  <option value="" disabled>Select Wholesaler</option>
                  {wholesalers.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}
                </select>
                <label className="flex items-center gap-1"><input type="checkbox" checked={newProd.vat_included} onChange={e=>setNewProd({...newProd,vat_included:e.target.checked})}/> VAT</label>
                <button type="submit" className="text-green-600"><PlusCircle/></button>
              </div>
            </form>
            <table className="w-full bg-white shadow rounded text-sm">
              <thead><tr className="bg-gray-100"><th>Name</th><th>Wholesaler</th><th>Qty</th><th>Price</th><th>VAT</th><th>Actions</th></tr></thead>
              <tbody>{products.map(p=>(<tr key={p.id} className="border-t">{editProdId===p.id?(<><td><input className="border px-1 w-full" value={editProdData.name||''} onChange={e=>setEditProdData({...editProdData,name:e.target.value})}/></td><td><select className="border px-1" value={editProdData.wholesaler_id||''} onChange={e=>setEditProdData({...editProdData,wholesaler_id:e.target.value})}>{wholesalers.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select></td><td><input type="number" className="border px-1 w-16" value={editProdData.qty_in_box||''} onChange={e=>setEditProdData({...editProdData,qty_in_box:+e.target.value})}/></td><td><input type="number" className="border px-1 w-20" value={editProdData.purchase_price||''} onChange={e=>setEditProdData({...editProdData,purchase_price:+e.target.value})}/></td><td className="text-center"><input type="checkbox" checked={editProdData.vat_included||false} onChange={e=>setEditProdData({...editProdData,vat_included:e.target.checked})}/></td><td className="flex gap-2"><button onClick={()=>saveEditProd(p.id)}><Save/></button><button onClick={()=>setEditProdId(null)}><X/></button></td></>):(<><td>{p.name}</td><td>{wholesalers.find(w=>w.id===p.wholesaler_id)?.name||'—'}</td><td>{p.qty_in_box}</td><td>£{p.purchase_price.toFixed(2)}</td><td className="text-center">{p.vat_included?'✓':'✗'}</td><td className="flex gap-2"><button onClick={()=>startEditProd(p)}><Pencil/></button><button onClick={()=>deleteProduct(p.id)}><Trash2/></button></td></>)}</tr>))}</tbody>
            </table>
          </section>
        )}
        ...
        {view==='sourcing'&&(
          <section>
            ...
            <div className="flex space-x-2 mb-4">
              {['all', ...wholesalers.map(w=>w.id)].map(tab=>(
                <button key={tab} onClick={()=>setActiveTab(tab)} className="px-3 py-1 bg-white shadow rounded">
                  {tab==='all'?'All':wholesalers.find(w=>w.id===tab)?.name}
                </button>
              ))}
            </div>
            <button onClick={()=>setShowModal(true)} className="mb-4 px-4 py-2 bg-green-600 text-white rounded">Add Sourcing Product</button>
            {showModal&&(
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                <div className="bg-white p-6 rounded shadow-lg w-3/4 max-w-lg">
                  <h3 className="text-xl font-bold mb-4">New Sourcing Item</h3>
                  <form onSubmit={addSource} className="space-y-4">
                    <div><label className="block font-semibold">Wholesaler:</label><select className="w-full border px-2 py-1 rounded" value={newSource.wholesaler_id} onChange={e=>setNewSource({...newSource,wholesaler_id:e.target.value})}><option value="all">All</option>{wholesalers.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select></div>
                    <div><label className="block font-semibold">Product Name:</label><input type="text" className="w-full border px-2 py-1 rounded" value={newSource.name} onChange={e=>setNewSource({...newSource,name:e.target.value})}/></div>
                    <div><label className="block font-semibold">Box Price:</label><input type="number" className="w-full border px-2 py-1 rounded" value={newSource.box_price} onChange={e=>setNewSource({...newSource,box_price:+e.target.value})}/></div>
                    <div><label className="block font-semibold">Items In Box:</label><input type="number" className="w-full border px-2 py-1 rounded" value={newSource.items_in_box} onChange={e=>setNewSource({...newSource,items_in_box:+e.target.value})}/></div>
                    <div><label className="block font-semibold">Wholesale Price:</label><input type="number" className="w-full border px-2 py-1 rounded" value={newSource.wholesale_price} onChange={e=>setNewSource({...newSource,wholesale_price:+e.target.value})}/></div>
                    <div><label className="block font-semibold">ASIN:</label><input type="text" className="w-full border px-2 py-1 rounded" value={newSource.asin} onChange={e=>setNewSource({...newSource,asin:e.target.value})}/></div>
                    <div><label className="block font-semibold">Sell Price:</label><input type="number" className="w-full border px-2 py-1 rounded" value={newSource.sell_price} onChange={e=>setNewSource({...newSource,sell_price:+e.target.value})}/></div>
                    <div><label className="block font-semibold">Amazon Fees:</label><input type="number" className="w-full border px-2 py-1 rounded" value={newSource.amazon_fees} onChange={e=>setNewSource({...newSource,amazon_fees:+e.target.value})}/></div>
                    <div className="flex justify-end space-x-2"><button type="button" onClick={()=>setShowModal(false)} className="px-4 py-2">Cancel</button><button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded">Save</button></div>
                  </form>
                </div>
              </div>
            )}
            <table className="w-full bg-white shadow rounded text-sm">
              <thead><tr className="bg-gray-100"><th>Name</th><th>Box Price</th><th>Qty</th><th>Wholesale</th><th>Sell</th><th>Fees</th><th>Profit</th><th>ROI</th></tr></thead>
